...
module.exports = new Component({
  ...
  run: async (client, interaction) => {
    ...
    if (!["report_ingame", "report_discord"].includes(selectedValue)) {
    ...
    const type = selectedValue === "report_ingame" ? "ingame" : "discord";
    const modalId = `report_${type}_submit_${Date.now()}_${interaction.user.id}`;

    // Create modal with more descriptive placeholders
    const modal = new ModalBuilder()
      .setCustomId(modalId)
      .setTitle(`Creating Ticket: ${type === "ingame" ? "Ingame" : "Discord"} Report`);

    const subjectInput = new TextInputBuilder()
      .setCustomId('ticket_subject')
      .setLabel('Subject')
      .setStyle(TextInputStyle.Short)
      .setPlaceholder(`Enter a brief summary of the offense`)
      .setRequired(true)
      .setMinLength(2)
      .setMaxLength(90);
    ...
    const bodyInput = new TextInputBuilder()
      ...
      .setPlaceholder('Include username of offender and evidence (screenshots, logs, etc.)')
      .setRequired(true)
      .setMinLength(10)
      .setMaxLength(1000);
    ...
    try {
      ...
      if (error.code === 'InteractionCollectorError') {
        console.log(`Modal timed out for ${type} report from user ${interaction.user.id}`);
